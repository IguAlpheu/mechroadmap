...
export default function LoginPage({ onLogin, authed }: LoginPageProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (mode === "register") {
      ...
      if (!getAuth()) { setError("No account found. Create one first."); setMode("register"); return; }
      setLoading(true);
      await new Promise((r) => setTimeout(r, 350));
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4 bg-background overflow-hidden">
      ...
      <button onClick={() => navigate("/")} className="absolute top-5 left-5 z-10 flex items-center gap-2 text-sm text-white/35 hover:text-white/70 transition-colors">
      ...
      <div className="relative z-10 w-full max-w-sm"
        ...
        <div className="lumeo-card rounded-2xl p-8 space-y-6">
          <div className="text-center space-y-3">
            <div className="flex justify-center">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                style={{ background: "linear-gradient(135deg, oklch(60% 0.22 240), oklch(50% 0.24 240))", boxShadow: "0 0 20px oklch(58% 0.22 240 / 35%)" }}>
                <Zap className="w-5 h-5 text-white" strokeWidth={2.5} />
              </div>
            </div>
            <div>
              <h1 className="text-white font-bold text-xl">Lumeo</h1>
              <p className="text-white/35 text-sm mt-0.5">{mode === "register" ? "Create your account" : "Welcome back"}</p>
            </div>
          </div>

          <div className="space-y-3">
            <div>
              <label className="block text-xs text-white/35 mb-1.5 font-medium">Username</label>
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleSubmit()}
                placeholder="yourname" className="lumeo-input" autoFocus disabled={loading} />
            </div>
            <div>
          ...
          <button onClick={handleSubmit} disabled={loading} className="lumeo-btn w-full flex items-center justify-center gap-2">
            {loading
          ...
          <p className="text-center text-xs text-white/30">
            {mode === "register" ? (<>Already have an account?{" "}<button onClick={() => { setMode("login"); setError(""); }} className="text-primary hover:underline">Sign in</button></>) : (<>No account?{" "}<button onClick={() => { setMode("register"); setError(""); }} className="text-primary hover:underline">Create one</button></>)}
          </p>
        </div>
        <p className="text-center text-xs text-white/15 mt-5">Your data is stored locally on your device.</p>
      </div>
    </div>
  );
}
